...
import { generalFeedSlice, initialState } from './slice';
import { onClose, onConnecting, onError, onMessage, onOpen } from './actions';

describe('generalFeedSlice', () => {
	it('should return the initial state', () => {
		expect(generalFeedSlice.reducer(undefined, { type: '' })).toEqual(
			initialState
		);
	});

	it('should return WebsocketStatus.CONNECTING and error == null after onConnecting action', () => {
		expect(generalFeedSlice.reducer(undefined, { type: onConnecting })).toEqual(
			{
				...initialState,
	...
	it('should return WebsocketStatus.ONLINE and error == null after onOpen action', () => {
		expect(generalFeedSlice.reducer(undefined, { type: onOpen })).toEqual({
			...initialState,
	...
	it('should return WebsocketStatus.OFFLINE and error == null after onClose action', () => {
		expect(generalFeedSlice.reducer(undefined, { type: onClose })).toEqual({
			...initialState,
	...
	it('should return error after onError action', () => {
		...
		expect(
			generalFeedSlice.reducer(undefined, {
				type: onError,
				payload: error,
			})
		).toEqual({
